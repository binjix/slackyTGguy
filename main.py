...
UNANSWERED_THRESHOLD = 10  # 1 minute for testing
...
async def check_unanswered_messages(context: ContextTypes.DEFAULT_TYPE):
    ...
    for msg_id, msg_data in list(messages.items()):
        time_diff = current_time - msg_data['timestamp']
        print(f"Message ID: {msg_id}, Time difference: {time_diff} seconds")
        if not msg_data['answered'] and time_diff > UNANSWERED_THRESHOLD:
            print(f"Threshold exceeded. Handling unanswered message: {msg_data['text']}")
            handle_unanswered_message(msg_id, msg_data)
        else:
            print(f"Message not yet eligible for handling. Answered: {msg_data['answered']}, Time diff: {time_diff}")
    print("Check complete.")


def handle_unanswered_message(msg_id, msg_data):
    print(f"Handling unanswered partner message: {msg_data['text']}")
    ...
    if output_id:
        ...
        print(f"Failed to send message to {OUTPUT_PLATFORM}. Error in notify_output function.")
...
def notify_slack(message_text, chat_title, user_name, message_link):
    try:
        ...
        print(f"Attempting to send message to Slack: {message_text}")
...
def main():
    ...
    if job_queue:
        job_queue.run_repeating(check_unanswered_messages, interval=10)
    else:
        print("Warning: JobQueue is not available. Periodic checks will not run.")
